...
`PIPELINES_ROOT` should be the parent folder that contains `nextflow-chipseq` and all `nf-*` module folders. If it is left unset, the launcher defaults to the parent directory of `nextflow-chipseq`.
...
By default, downstream sheets are auto-generated from `SAMPLES_MASTER`. Keep optional sheet vars empty unless you intentionally want manual override. If an optional sheet path is set, the launcher now requires that file to exist before starting.
...
## Available Module Walkthroughs

- `docs/NEXTFLOW_CHIPSEQ_WALKTHROUGH_01_FASTQC.md`
- `docs/NEXTFLOW_CHIPSEQ_WALKTHROUGH_02_FASTP.md`
- `docs/NEXTFLOW_CHIPSEQ_WALKTHROUGH_03_BWA.md`
- `docs/NEXTFLOW_CHIPSEQ_WALKTHROUGH_04_PICARD.md`
- `docs/NEXTFLOW_CHIPSEQ_WALKTHROUGH_05_CHIPFILTER.md`
- `docs/NEXTFLOW_CHIPSEQ_WALKTHROUGH_06_MACS3.md`
...
- `nf-macs3` applies peak-level blacklist filtering only when `MACS3_PEAK_BLACKLIST_BED` is set.
...
- `nf-deeptools-heatmap` current workflow is auto-driven from `samples_master + chipfilter + macs3(strict) + diffbind`; keep `RUN_DEEPTOOLS_HEATMAP=false` when DiffBind is disabled.
...
- User-set optional sheet paths are validated before starting; missing files fail fast instead of silently falling back to auto mode.
